                ...
                if source_file.exists():
                    ...
                    if dest_file.exists():
                        if filecmp.cmp(str(source_file), str(dest_file), shallow=False):
                            needs_copy = False
                            log_info(logger, f"Python script unchanged: {py_file}")
...
'''
            ...
            if source_docker_compose.exists():
                ...
                if os.path.exists(dest_docker_compose):
                    if filecmp.cmp(str(source_docker_compose), dest_docker_compose, shallow=False):
                        needs_copy = False
                        log_info(logger, "Docker Compose unchanged, skipping copy")

                if needs_copy:
                    subprocess.run(
                        ['cp', '-p', str(source_docker_compose), dest_docker_compose], check=True)
                    log_success(logger, "Docker Compose copied to: ~/docker-compose.yml")
            else:
                log_warning(logger, "Docker Compose file not found")
    ...
    def clean_swap_interactive(self):
        ...
        swap_wait_seconds = int(os.getenv('SWAP_CLEANUP_WAIT_SECONDS', 5))
        ...
        try:
            ...
            console.print(f"[cyan]Waiting {swap_wait_seconds} seconds before re-enabling...[/cyan]")
            ...
            log_swap(logger, f"SWAP cleanup completed (waited {swap_wait_seconds}s between operations)")
